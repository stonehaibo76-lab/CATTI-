...
const Sidebar: React.FC<SidebarProps> = ({ currentView, onViewChange }: SidebarProps) => {
  ...
  const saveSettings = () => {
    ...
    // Reload removed to prevent interface disappearing issues
    // Services read from localStorage dynamically, so reload is not strictly required
  };
    ...
    <aside className="w-64 bg-white border-r border-slate-200 h-screen sticky top-0 flex flex-col z-40">
      ...
      {showSettings && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="w-full max-w-lg bg-white rounded-[2.5rem] p-8 shadow-2xl relative animate-in zoom-in-95">
            ...
            <p className="text-slate-500 mb-8 font-medium text-sm">选择你的智能驱动引擎</p>
              ...
              {provider === 'deepseek' && (
                <div className="space-y-3 animate-in slide-in-from-top-2">
                  ...
                  <input 
                    type="password" 
                    placeholder="sk-..." 
                    className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:ring-4 focus:ring-blue-100 font-mono text-sm transition-all"
                    value={deepseekKey}
                    onChange={(e) => setDeepseekKey(e.target.value)}
                  />
                  <p className="text-[10px] text-slate-400">
                    Key 仅存储在本地浏览器中，用于直接请求 api.deepseek.com。
                    <br/>DeepSeek 暂不支持语音合成(TTS)，朗读功能将继续使用 Gemini。
                  </p>
                </div>
              )}
